...
#________________________Modelo regresivo lineal SVM afinado____________________
...
tuneResult1 <- tune(svm, y ~ x,  data = Data,
                    ranges = list(cost = costo_lineal, nu = nu))
...
tuneResult <- tune(svm, y ~ x, data = Data,
                   ranges = list(cost = costo_lineal_fino, 
                                 nu = nu_fino))
...
best_model <- svm(y ~ x, data = Data, cost = resultados_pso$par[1], nu = resultados_pso$par[2])

# Definir el rango de búsqueda para los hiperparámetros para la segunda iteración de PSO

lo <- c(best_model$cost-100, best_model$nu-0.05)
hi <- c(best_model$cost+100, best_model$nu+0.05)
