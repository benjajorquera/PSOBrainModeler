...
resultados <- foreach(seed = seeds, .combine = 'c', .packages = c('dplyr', 'tseries')) %dopar% {
  
  brain_modeler_config <-
    configure_pso_brain_modeler(vsvr_tolerance = 1, seed = seed)
