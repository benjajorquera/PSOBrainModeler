...
svm_model <- svm(mabp_norm_1 ~ time_1, kernel = "linear", type = "nu-regression")
...
nu <- seq(0.1, 0.9, 0.1)
cost <- c(0.25, 292.8, 585.36, 877.91, 1170.46, 1463.02, 1755.57, 2048.13, 2340.68,
...
objetivo <- function(x) {
  ...
  svm_model <- svm(mabp_norm_1 ~ time_1, cost = cost, nu = nu, kernel="linear",
                   type = "nu-regression")
...
best_model <- svm(mabp_norm_1 ~ time_1, cost = resultados_pso$par[1], nu = resultados_pso$par[2], kernel="linear",
                  type = "nu-regression")
...
points(time_1, predicted_labels, col = "purple", pch=4)
