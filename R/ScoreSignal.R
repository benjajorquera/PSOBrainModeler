...
#' @export
evaluate_signal_quality <- function(signal, pressure_start_point) {
  ...
  if (!(global_min %in% peak_range) ||
    ...
    warning("RESPONSE SIGNAL FAILED BASIC FILTER", call. = FALSE)
    return(-10)
  }
  ...
  peak_after_drop <- max(signal[15:30]) - utils::head(peak_range, 1)
