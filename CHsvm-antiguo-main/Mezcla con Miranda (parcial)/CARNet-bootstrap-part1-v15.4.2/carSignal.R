require(signal)     # butter filter
# require(pracma)     # interp1
...
{
  if(!is.divisible(time.until.release, sampling.time))
    stop("time until release must be a multiple of the target sampling time")
  if(!is.divisible(time.after.release, sampling.time))
    stop("time after release must be a multiple of the target sampling time")
	
	frequency <- 1 / sampling.time
  nsamples.stabilisation.left <- round(left.stabilisation.time / sampling.time)
  nsamples.until.release <- round(time.until.release / sampling.time) + 1
...
normalise.signal <- function(signal,
...
get.best.templates <- function(time.instants,
                               signal,
                               templates,
                               ...
                               ...)
{
  ...
  i <- which(are.tolerably.equal(time.instants, referential.time.instant, time.tol))
  if(length(i) != 1)
  ...
  i <- which(are.tolerably.equal(time.instants, ans[["initial.time.instant"]], time.tol))
  if(length(i) != 1)
  ...
  ans[["final.time.instant"]] <- referential.time.instant + delta.time.after.ref
  i <- which(are.tolerably.equal(time.instants, ans[["final.time.instant"]], time.tol))
  if(length(i) != 1)
    stop("final time instant could not be found in specified time instants\n")
  ans[["final.sample"]] <- i
  
  # Sets relevant interval
  ans[["relevant.samples"]] <- ans[["initial.sample"]]:ans[["final.sample"]]
  
  # Gets relevant segments
  .get.relevant.signal <- function(s) s[ans[["relevant.samples"]]]
  ans[["relevant.signal.segment"]] <- .get.relevant.signal(ans[["signal"]])
  ans[["relevant.template.segments"]] <- lapply(ans[["templates"]], .get.relevant.signal)
  
  # Gets fit values
  .get.fit.value <- function(t) comparison.function(ans[["relevant.signal.segment"]], t, ...)
  ans[["fit.values"]] <- sapply(ans[["relevant.template.segments"]], .get.fit.value)
